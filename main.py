...
def drive(angle):
    ...
    if STOP:
        ...
        #elif turning:
            #if (clockwize): angle = straight_angle - 145
            #elif (not(clockwize)): angle = straight_angle + 145
...
while(True):
    ...
    if red_ang_green_cubes:
        i = 0
        b_led.off()
        for blob in img.find_blobs(thresholds, pixels_threshold=20, area_threshold=20, roi = (0, 17, 1, 1)): # 160, 80
